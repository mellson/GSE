...
This sensor works by looking up the machines registered ip address, or if available uses the machines build-in gps, and uses this to acquire an geographical address.
Knowing the whereabouts of the machine neither infer presence nor interruptibility on its own. 
...
Unlike the other sensors, the webcam can sense if a person is setting in front of the machine, even if the person is not directly interacting with the machine.
But even though the webcam can infer non-presence, there are still some issues.
For example, if a person is located in his/her office but not facing the machine e.g. sitting in a chair across the office, the person will be present and possible available, but the webcam will infer that it cannot detect a presence.
...
If we assume that a person always carries his/her mobile device with them, then the bluetooth or wifi beacon from this device can be used to infer if the person is in proximity of the machine.
...
The issue here is if the person should forget his/her mobile device, ether nearby the machine or at home. 

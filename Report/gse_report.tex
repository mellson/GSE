...
First we recorded two test persons in their work environment with a camera, while Approximators sensors collected data.
...
On this webpage the participants were presented for short sequences of video, showing the test subject in random selected situations in their work environment.

\subsubsection{Stage One}
The two test persons was both males, and \textless age\textgreater and \textless age\textgreater years of age, and both worked for a security company where their primary task was to maintain and develop on the company system.
Since the company also have offices in other countries, the test persons often did parallel development with the out-of-country teams.
...
The test persons was recorded with <insert camera specs here - along with dataformat\textgreater.
The camera was placed at an angle that did not allow for the content of their screen to be visible.
The angle did however show as much of the office as possible, while showing the face of the test person.
...
Approximator utilized the keyboard, mouse and webcam \textless possible more\textgreater as sensors, and sent data from these sensors to the infrastructure throughout the experiment.
Approximator was specially configured to prompt a visual bell for the test person at random intervals averaging at two times per hour.
The test persons was instructed to self-report their own level of interruptibility on a scale of 1 (highly-interruptible) to 5 (highly-non-interruptible) either verbally or by show of fingers on one hand when prompted.
The test persons had no way of pausing the application \textless still to be finally determided\textgreater, but could request retroactively to have sequence deleted if necessary.
The prompts where the test person was not present was removed from the dataset after the experiment.
At other sequences the test persons clearly registered the prompt, but did not signal their interruptibility, due to them being on the phone or engaged in a other activity that required their full attention. These were interpreted by us as a high-non-interruptible event.
...
At this point we created a webpage with the explicit purpose of peer reviewing the video we had collected. The link was sent out to persons from the software development community, who then partook in the survey on their own machine and in their own setting.
When the persons entered the site, they were first required to enter their demographic information.
...
This is to insure that the persons participating in the survey are familiar with the environmental setting of software development and also to insure the validity of this paper.
Before the person was presented for the sequences, they were instructed to imagine themselves in the office trying to interact with the person in the sequence, and based on a 1 (highly-interruptible) to 5 (highly-non-interruptible) scale assess how interruptible they were.

The person was then presented with \textless insert number of samples\textgreater sequences of 15-30 seconds in length.
These was the 15-30 seconds leading up to/or after the test person being prompted for self-report.
The person could replay the sequence as many times as they wanted until they felt they could give a valid assessment.
But each sequence was played in full length before the person could rate it and proceed to the next.
